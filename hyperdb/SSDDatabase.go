...
func NewSSDatabase() (*SSDatabase, error) {
	...
	rdP := redis.NewPool(func() (redis.Conn, error) {
		return redis.Dial("tcp", ":"+strconv.Itoa(ssdbProxyPort), redis.DialConnectTimeout(time.Duration(ssdbTimeout)*time.Second))
	}, ssdbPoolSize)
...
func (it *IteratorSsdb) Seek(key []byte) bool {
	...
	if !it.iterator.Seek(key) {
		...
		if it.ssdbNow < it.ssdbNum {
			...
			it.port += ssdbGap
			rdP := redis.NewPool(func() (redis.Conn, error) {
				return redis.Dial("tcp", ":"+strconv.Itoa(it.port), redis.DialConnectTimeout(time.Duration(ssdbTimeout)*time.Second))
			}, ssdbPoolSize)
...
func (it *IteratorSsdb) Next() bool {
	if !it.iterator.Next() {
		...
		if it.ssdbNow < it.ssdbNum {
			...
			it.port += 10
			rdP := redis.NewPool(func() (redis.Conn, error) {
				return redis.Dial("tcp", ":"+strconv.Itoa(it.port), redis.DialConnectTimeout(60*time.Second))
			}, 10)
			itertaor2 := &IteratorImp{
				Pool:      rdP,
				ListKey:   make([]string, ssdbIteratorSize, ssdbIteratorSize),
...
func (ssdb *SSDatabase) NewIterator(prefix []byte) Iterator {
	rdp := redis.NewPool(func() (redis.Conn, error) {
		return redis.Dial("tcp", ":"+strconv.Itoa(ssdb.port), redis.DialConnectTimeout(time.Duration(ssdbTimeout)*time.Second))
	}, ssdbPoolSize)
