...
func TestDes3(t *testing.T) {
	...
	for _,i:=range set{
		start := time.Now()
		for j:=0;j<i;j++{
			TripleDesEncrypt([]byte("polaris@studygol"), key)
		}
		fmt.Printf("TripleDesEncrypt test %d: %s",i,time.Since(start))
		fmt.Println()
	}
	result, _ := TripleDesEncrypt([]byte("polaris@studygol"), key)
	for _,i:=range set{
		start := time.Now()
		for j:=0;j<i;j++{
			TripleDesDecrypt(result, key)
		}
		fmt.Printf("TripleDesDecrypt test %d: %s",i,time.Since(start))
		fmt.Println()
	}
