...
func TestRevertToTargetBlock(t *testing.T) {
	...
	state.Bucket_tree.AddChangesForPersistence(writeBatch,big.NewInt(1))
	writeBatch.Write()
	...
	logger.Debugf("after revert from %d",fromBlock," to %d",toBlock,"--------------the state.GetHash() is ",(hash_revert0))
