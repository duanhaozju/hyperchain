...
func (pbft *pbftImpl) msgToEvent (msg *ConsensusMessage) (interface{}, error) {
	...
	if err != nil {
		logger.Errorf("Unmarshal error, can not unmarshal %v, error: %v", msg.Type, err)
		return nil, err
	}
...
func (pbft *pbftImpl) dispatchLocalEvent(e *LocalEvent) events.Event {
	...
	default:
		logger.Errorf("Not Supported event: %v", e)
		return nil
	}
}
...
func (pbft *pbftImpl) handleCorePbftEvent(e *LocalEvent) events.Event {
	...
	case CORE_BATCH_TIMER_EVENT:
		...
		if atomic.LoadUint32(&pbft.activeView) == 1 {
			return pbft.batchMgr.sendBatchRequest()
		}
		return nil
	...
	default:
		logger.Errorf("Invalid core pbft event : %v", e)
		return nil
	}
}
...
func (pbft *pbftImpl) handleViewChangeEvent(e *LocalEvent) events.Event {
	...
	case VIEW_CHANGE_TIMER_EVENT:
		logger.Warningf("Replica %d view change timer expired, sending view change: %s", pbft.id, pbft.vcMgr.newViewTimerReason)
	...
	case VIEW_CHANGE_VC_RESET_DONE_EVENT:
		...
		if e.Event.(protos.VcResetDone).SeqNo != pbft.h+1 {
			logger.Warningf("Replica %d finds error in VcResetDone, expect=%d, but get=%d", pbft.id, pbft.h+1, e.Event.(protos.VcResetDone).SeqNo)
			return nil
		}
	...
	default:
		logger.Errorf("Invalid view change event event : %v", e)
		return nil
	}
	return nil
}
...
func (pbft *pbftImpl) handleRecoveryEvent(e *LocalEvent) events.Event {
	...
	case RECOVERY_DONE_EVENT:
		logger.Criticalf("======== Replica %d finished recovery, height: %d", pbft.id, pbft.exec.lastExec)
	...
	case RECOVERY_RESTART_TIMER_EVENT:
		...
		return nil
	default:
		logger.Errorf("Invalid recovery service events : %v", e)
		return nil
	}
}
...
func (pbft *pbftImpl) dispatchConsensusMsg(e events.Event) events.Event {
	logger.Debugf("start processing consensus mesage")
