...
// pbftImpl is the core struct of pbft module, which handles all functions about consensus
type pbftImpl struct {
	...
	status 		PbftStatus // keep all basic status of pbft in this object
...
func (pbft *pbftImpl) dispatchCorePbftMsg(e events.Event) events.Event {
	switch et := e.(type) {
	case *PrePrepare:
		if et.SequenceNumber != uint64(0) {// The seqNo of prePrepare-sp message is 0
...
func (pbft *pbftImpl) trySendPrePrepares() {
	...
	for stopTry := false; !stopTry; {
		if find, txBatch, digest, resultHash := pbft.findNextPrePrepareBatch(); find { // if find, just send
...
func (pbft *pbftImpl) findNextPrePrepareBatch() (bool, *TransactionBatch, string, string) {
	...
	for digest = range pbft.batchVdr.cacheValidatedBatch {
		...
		if cache.vid != pbft.batchVdr.lastVid+1 { // find the batch whose vid is exactly lastVid+1
		...
		// if the batch is full of transactions which cannot pass validate,
		// send a special prePrepare whose seqNo is 0
...
// sendPrePrepareSp send special prepare message. If all transactions in a batch cannot pass validation,
...
func (pbft *pbftImpl) sendPrePrepareSp(digest string, hash string, batch *TransactionBatch) {
	...
	preprep := &PrePrepare{
		View:           pbft.view,
		SequenceNumber: uint64(0), // special prePrepare's seqNo is set to 0
...
func (pbft *pbftImpl) maybeSendCommit(digest string, v uint64, n uint64, vid uint64) error {
	...
	if ok := pbft.isPrimary(); ok {
		...
		if !cert.sentValidate {// if haven't send to validate the batch
...
func (pbft *pbftImpl) recvCommit(commit *Commit) error {
	...
	if pbft.committed(commit.BatchDigest, commit.View, commit.SequenceNumber) {
		pbft.stopNewViewTimer()// stop new view timer which was started when recv prePrepare
...
// commitTransactions commit all available transactions by order
...
func (pbft *pbftImpl) processTransaction(tx *types.Transaction) events.Event {
	...
	// this function processes transaction from client, so should check poolSize
...
func (pbft *pbftImpl) checkpoint(n uint64, info *protos.BlockchainInfo) {
	...
	pbft.recvCheckpoint(chkpt)	// send to itself
...
func (pbft *pbftImpl) recvValidatedResult(result protos.ValidatedTxs) error {
	...
	if primary == pbft.id {
		...
		if result.SeqNo == uint64(0) {
			if len(result.Transactions) == 0 { // if this is a special batch with no valid transaction
