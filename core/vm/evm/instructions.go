...
func jump(mapping map[uint64]uint64, destinations map[uint64]struct{}, context vm.VmContext, to *big.Int) (uint64, error) {
	if !validDest(destinations, to) {
		nop := OpCode(context.GetOp(to.Uint64()))
...
func (instr instruction) do(program *Program, pc *uint64, env vm.Environment, context vm.VmContext, memory *Memory, stack *stack) ([]byte, error) {
	...
	switch instr.op {
	case JUMP:
		if pos, err := jump(program.mapping, program.destinations, context, stack.pop()); err != nil {
			return nil, err
		} else {
			*pc = pos
			return nil, nil
	...
	case JUMPI:
		...
		if cond.Cmp(common.BigTrue) >= 0 {
			if pos, err := jump(program.mapping, program.destinations, context, pos); err != nil {
				return nil, err
			} else {
				*pc = pos
				return nil, nil
	...
	default:
		...
		instr.fn(instr, pc, env, context, memory, stack)
	}
	*pc++
	return nil, nil
}
...
func opCodeCopy(instr instruction, pc *uint64, env vm.Environment, context vm.VmContext, memory *Memory, stack *stack) {
	...
	codeCopy := getData(context.GetCode(), cOff, l)
...
func opLog(instr instruction, pc *uint64, env vm.Environment, context vm.VmContext, memory *Memory, stack *stack) {
	...
	log := NewLog(context.Address(), topics, d, env.BlockNumber().Uint64())
	env.AddLog(log)
}
...
func opCreate(instr instruction, pc *uint64, env vm.Environment, context vm.VmContext, memory *Memory, stack *stack) {
	var (
		...
		gas          = new(big.Int).Set(context.GetGas())
	)
	context.UseGas(context.GetGas())
	_, addr, suberr := env.Create(context, input, gas, context.GetPrice(), value)
...
func opCall(instr instruction, pc *uint64, env vm.Environment, context vm.VmContext, memory *Memory, stack *stack) {
	...
	ret, err := env.Call(context, address, args, gas, context.GetPrice(), value, 0)
...
func opCallCode(instr instruction, pc *uint64, env vm.Environment, context vm.VmContext, memory *Memory, stack *stack) {
	...
	ret, err := env.CallCode(context, address, args, gas, context.GetPrice(), value)
...
func opDelegateCall(instr instruction, pc *uint64, env vm.Environment, context vm.VmContext, memory *Memory, stack *stack) {
	...
	args := memory.Get(inOffset.Int64(), inSize.Int64())
	ret, err := env.DelegateCall(context, toAddr, args, gas, context.GetPrice())
	if err != nil {
		stack.push(new(big.Int))
	} else {
		stack.push(big.NewInt(1))
