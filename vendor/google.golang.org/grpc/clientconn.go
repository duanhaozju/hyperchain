...
func (ac *addrConn) resetTransport(drain bool) error {
	grpclog.Warning("reset Transport $$$$$")
	ac.mu.Lock()
	...
	for retries := 0; ; retries++ {
		...
		if err != nil {
			...
			// if is err. broken pipe set a limit to recreate the connection
			grpclog.Warningf("clientconn.go: 973 grpc: addrConn.resetTransport failed to create client transport: %v; Reconnecting to %v", err, ac.addr)
			ac.mu.Lock()
			if ac.state == Shutdown {
				// ac.tearDown(...) has been invoked.
				ac.mu.Unlock()
...
func (ac *addrConn) transportMonitor() {
	for {
		...
		case <-t.Error():
			...
			default:
				//TODO IF ADD THIS, IT WILL have some effect
				grpclog.Warning("XSAXAS")
				t.Close()
				return
			}
			ac.mu.Lock()
